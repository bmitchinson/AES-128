...
def invMixColumns(state, f):
    ...
    for oldCol in oldCols:
        tmp = [
            hex(
                f.Multiply(14, oldCol[0])
                ^ f.Multiply(11, oldCol[1])
                ^ f.Multiply(13, oldCol[2])
                ^ f.Multiply(9, oldCol[3])
            ),
            hex(
                f.Multiply(9, oldCol[0])
                ^ f.Multiply(14, oldCol[1])
                ^ f.Multiply(11, oldCol[2])
                ^ f.Multiply(13, oldCol[3])
            ),
            hex(
                f.Multiply(13, oldCol[0])
                ^ f.Multiply(9, oldCol[1])
                ^ f.Multiply(14, oldCol[2])
                ^ f.Multiply(11, oldCol[3])
            ),
            hex(
                f.Multiply(11, oldCol[0])
                ^ f.Multiply(13, oldCol[1])
                ^ f.Multiply(9, oldCol[2])
                ^ f.Multiply(14, oldCol[3])
            ),
        ]
        for j in range(0, 4):
            tmp[j] = "{0:#0{1}x}".format(int(tmp[j], 16), 4)[2:]
